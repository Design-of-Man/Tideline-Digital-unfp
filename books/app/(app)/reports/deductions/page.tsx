  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6 max-w-md">
        <Card className="p-5">
          <div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Total spent</div>
          <div className="text-2xl font-semibold mt-1.5 text-foreground">{money(totalSpent)}</div>
        </Card>
        <Card className="p-5">
          <div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Deductible</div>
          <div className="text-2xl font-semibold mt-1.5 text-success">{money(totalDeductible)}</div>
        </Card>
      </div>
            ...
            <tbody>
              {data.map((r, i) => (
                <tr key={r.tax_category_id ?? `none-${i}`}>
                  <td className="font-medium text-foreground">{r.category_name ?? "Uncategorized"}</td>
                  <td className="text-muted-foreground text-xs">{r.form_line ?? "—"}</td>
                  <td className="num tabular-nums text-muted-foreground">{money(r.total_amount)}</td>
                  <td className="num tabular-nums text-success">{money(r.deductible_amount)}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
